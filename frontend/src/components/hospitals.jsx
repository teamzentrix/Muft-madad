...
// ✅ Accepts optional `city` prop — if provided, filters by city
export default function HospitalsSection({ city = '' }) {
  ...
  const fetchHospitals = async () => {
    ...
    setError('');
    try {
      // If city provided, filter by city; otherwise fetch all
      const url = city
    ...
    <section className="py-12 px-4 bg-gradient-to-b from-gray-50 to-white">
      <div className="max-w-7xl mx-auto">
        ...
        {!loading && !error && visibleHospitals.length > 0 && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
              {visibleHospitals.map(h => <HospitalCard key={h.id || h.uuid} hospital={h} />)}
            </div>
            {hasMore && (
