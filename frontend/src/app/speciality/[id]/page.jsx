...
export default function SpecialityPage({ params }) {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        setLoading(true);
        const specRes = await axios.get(`http://localhost:4000/api/specialities/${id}`, { withCredentials: true });
        setSpeciality(specRes.data.data);

        const treatRes = await axios.get(`http://localhost:4000/api/admin/getBySpecialty/${id}`, { withCredentials: true });
        const treatData = treatRes.data;
        setTreatments(Array.isArray(treatData) ? treatData : (treatData.data || []));

        try {
          const hospRes = await axios.get(`http://localhost:4000/api/hospitals?specialty=${id}`, { withCredentials: true });
          setHospitals(hospRes.data.data || []);
        } catch { setHospitals([]); }

        try {
          const docRes = await axios.get(`http://localhost:4000/api/doctors?specialty=${encodeURIComponent(specRes.data.data?.name_en || '')}`, { withCredentials: true });
          ...
          <div className="hero-inner">
            ...
            {speciality?.image && (
              <div className="hero-img-wrap">
                <img src={speciality.image} alt={specialityName} className="hero-img" />
              </div>
            )}
          </div>
        </section>
