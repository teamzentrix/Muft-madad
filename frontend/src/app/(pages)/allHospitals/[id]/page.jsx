...
import {
  Phone, MapPin, Clock, Users, Stethoscope, Star,
  AlertCircle, RefreshCw, ChevronLeft, ChevronRight,
  CheckCircle, ZoomIn, X, Images, Shield, Building2, Pill
} from 'lucide-react';
...
const GalleryTab = ({ images }) => {
  ...
  return (
    <>
      <SectionHeading>Gallery <PillBadge>{images.length} photos</PillBadge></SectionHeading>
  ...
  <div className="space-y-8">
    ...
    <div>
      ...
      <div className="bg-gray-50 border border-gray-100 rounded-xl p-5 space-y-4">
        ...
        {h.timing_display && <InfoRow icon={Clock} label="Timings">{h.timing_display}</InfoRow>}
        {h.phone && (
...
const DoctorsTab = ({ doctors, loading }) => {
  if (loading) return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {[...Array(4)].map((_, i) => (
        <div key={i} className="animate-pulse flex gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
  ...
  <div>
    ...
    {items?.length > 0 ? (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {items.map((item, i) => (
          <div key={i} className="flex items-center gap-3 p-3.5 bg-purple-50 border border-purple-100 rounded-xl hover:border-purple-200 transition-colors">
      ...
      <EmptyState icon={<Stethoscope className="w-8 h-8" />} msg="No specialities listed." />
    )}
  </div>
);
...
const TreatmentsTab = ({ items }) => (
  <div>
    ...
    {items?.length > 0 ? (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {items.map((item, i) => (
          <div key={i} className="flex items-center gap-3 p-3.5 bg-orange-50 border border-orange-100 rounded-xl hover:border-orange-200 transition-colors">
      ...
      <EmptyState icon={<Pill className="w-8 h-8" />} msg="No treatments listed." />
    )}
  </div>
);
...
const ServicesTab = ({ items }) => (
  <div>
    ...
    {items?.length > 0 ? (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {items.map((item, i) => (
          <div key={i} className="flex items-center gap-3 p-3.5 bg-green-50 border border-green-100 rounded-xl hover:border-green-200 transition-colors">
      ...
      <EmptyState icon={<Building2 className="w-8 h-8" />} msg="No services listed." />
    )}
  </div>
);
...
export default function HospitalDetailPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API}/hospitals/${id}`, { withCredentials: true });
      const data = res.data?.data || res.data;
      setHospital(data);

      // 2. Fetch doctors whose currently_serving matches hospital name
      //    We do this after we have the hospital name
      if (data?.name) {
  ...
  const TABS = [
    ...
    { id: 'specialities', label: 'Specialities', count: hospital?.available_specialities?.length || 0 },
    { id: 'treatments',   label: 'Treatments',   count: hospital?.available_treatments?.length || 0 },
    { id: 'services',     label: 'Services',     count: hospital?.available_services?.length || 0 },
    { id: 'gallery',      label: 'Gallery',      count: hospital?.gallery_images?.length || 0 },
  ];

  const renderTab = () => {
    switch (activeTab) {
      case 'overview':     return <OverviewTab h={hospital} />;
      case 'doctors':      return <DoctorsTab doctors={doctors} loading={doctorsLoading} />;
      case 'specialities': return <SpecialitiesTab items={hospital?.available_specialities || []} />;
      case 'treatments':   return <TreatmentsTab items={hospital?.available_treatments || []} />;
      case 'services':     return <ServicesTab items={hospital?.available_services || []} />;
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-gray-500">
                  ...
                  {hospital.rating > 0 && (
                    <span className="flex items-center gap-1 text-yellow-600 font-medium">
                      ...
                      {hospital.total_reviews > 0 && <span className="text-gray-400 font-normal ml-0.5">({hospital.total_reviews})</span>}
                    </span>
                  )}
                </div>
              </div>
